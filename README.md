...
| Processor restart drill | `artifacts/failure-drills/restart-processor-20260423-182511.json` | batch size `25`, `3` processors, `2,000 eps` target; one replica restarted, lag recovered in `6.61s`, final lag `0` |
| Broker outage drill | `artifacts/failure-drills/broker-outage-20260423-182640.json` | batch size `25`, `2,000 eps` target; accepted traffic recovered in `4.13s`, but archive accounting gap was `38,592`, so this scenario remains degraded |
| PostgreSQL pause drill | `artifacts/failure-drills/pause-postgres-20260423-182821.json` | batch size `25`, `2,000 eps` target; processing resumed `0.05s` after Postgres became healthy, peak lag `5,357`, final lag `0` |
| Replay and rebuild drill | `artifacts/failure-drills/replay-archive-20260423153016.json` | `50` duplicate replays produced `0` source-metric overcount and rebuilt hot views, but the verifier observed only `47` rebuilt events, so this scenario remains degraded |
| Poison-message drill | `artifacts/failure-drills/inject-poison-message-20260423-153845.json` | malformed Kafka record produced `dead_letter_delta: 1`, lag stayed `0`, and the repaired DLQ path kept processors healthy |

Current local evidence is deliberately conservative. Batch ingest met the intermediate `2,000 processed eps` gate, and sharded tenant metrics moved the `5,000 processed eps` profile from a near miss to a `2/3` clean-state pass rate on this machine. The three clean-state 5k runs produced a processed-throughput range of `4,921.98` to `5,318.88 eps`. The 2026-04-23 fixes removed the raw archive as the dominant synchronous ingest bottleneck, reduced processor window/source write pressure, replaced one-event-per-request benchmark traffic with a production-shaped batch profile, split tenant aggregate writes across deterministic shards, and added explicit Kafka topic bootstrap for the main and DLQ topics. The batch-path failure reruns now show restart, Postgres pause, and poison-message recovery evidence, while broker outage accounting and replay rebuild verification still need work before stronger resilience claims are justified.
