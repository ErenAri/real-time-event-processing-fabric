...
- Remaining gap: the local flat-file archive is partitioned by date only; the drill scanned `670,695` records and skipped `670,670` to replay `25`, so large replay workloads need tenant/time indexing or object-prefix partitioning before this is efficient at production scale
