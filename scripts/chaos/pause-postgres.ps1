...
try {
    ...
    if (-not (Wait-ContainerHTTPReady -ContainerName $producerContainerName -TimeoutSeconds 20)) {
...
if (Test-Path $evidenceScript) {
    try {
        ...
        Write-Warning "Postgres pause drill completed, but evidence summary refresh failed: $($_.Exception.Message)"
    }
}
